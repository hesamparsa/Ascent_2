...
library(readr)
library(dplyr) 
df <- read_csv("data/2015_311.csv")
df_ozone <- read_csv("data/ad_viz_plotval_data_ozone.csv")
df_co <- read_csv("data/ad_viz_plotval_data_co.csv")
df_pm <- read_csv("data/ad_viz_plotval_data_pm2_5.csv")
df_man <- filter(df, Borough == "MANHATTAN") # You should use dplyr filter 
...
library(ggmap)
map <- get_map(location = "manhattan", zoom=12, maptype = "roadmap"
...
df_pm <- df_pm %>%
  group_by(Date) %>%
  dplyr:: summarise(pm=mean(pm)) # take average of 5 sensor
